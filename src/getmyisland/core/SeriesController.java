...
public class SeriesController {
	...
	private static void listEpisodes(final File root) {
		...
		// Create a series
		String seriesName = root.getName().substring(0, root.getName().length() - 9); //Remove the last 9 letters which are the releaseYears
		seriesName = seriesName.replaceAll("_", " ");
		String seriesReleaseYears = root.getName().substring(root.getName().length() - 8); // Get the last 4 digits which are the releaseYears
		String pathToSeriesCoverImage = root.getAbsolutePath() + "\\" + root.getName() + ".jpg";
		
		Series tempSeries = new Series(seriesName, seriesReleaseYears, pathToSeriesCoverImage, 0);
		...
		for (final File child : childs) {
			...
			if (child.getName().toLowerCase().endsWith(".mp4") || child.getName().toLowerCase().endsWith(".mkv")) {
				...
				String episodeName = tempName.substring(7, tempName.length()); // The last 4 letters are always the release year
				episodeName = episodeName.replaceAll("_", " ");
				
				String seasonNumberString = tempName.substring(1, 3); // The second and third letter are always the season number
				int seasonNumber = Integer.parseInt(seasonNumberString);
				seasonNumbers.add(seasonNumber);
				
				String episodeNumberString = tempName.substring(4, 6); // The second and third letter are always the season number
